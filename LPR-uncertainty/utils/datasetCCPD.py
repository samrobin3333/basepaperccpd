...
class DatasetCCPD:
    def __init__(self,data):
        ...
        self.ending = "hdf5"
        if self.dataset == "czech" or self.dataset == "ccpd":
            self.ending = "h5"
    ...
    def load_data_generator_cl(self,IDs_batch,batchsize, batchEnsemble, ensemble_size):
        X_batch = np.empty((batchsize, self.in_shape[0], self.in_shape[1], self.n_channels))
        l_batch = np.empty((batchsize, self.n_classes), dtype=int)
        for i,ID in enumerate(IDs_batch):
            X_batch[i,] = self.noiser((self.files['train']['images_hr'])[int(ID)])
            l_batch[i,] = (self.files['train']['labels'])[int(ID)]

        if batchEnsemble:
            X_batch = np.tile(X_batch, [ensemble_size, 1, 1, 1])
    ...
    def load_data(self,mode,num,sampling=False,range=False,start=0,end=-1):
        ...
        if num <= len(self.IDs[mode]) and num > 0:
            if sampling:
                ...
                self.IDs[mode] = self.IDs[mode][start:end]
            else:
